...
static void eprintf_local_uint_padding_left(char *nb_s, eprintf_mod_t *mod)
{
...
static void eprintf_local_uint_padding_right(char *nb_s, eprintf_mod_t *mod)
{
    (mod->modflag.plus) ? eappend_buff_str("+") : 0;
    eprintf_append_padding(mod->modflag.precision, true);
    eappend_buff_str(nb_s);
...
void eprintf_local_uint(va_list *ap, eprintf_mod_t *mod)
{
    ...
    if (mod->modflag.pad.right)
        eprintf_local_uint_padding_right(nb_s, mod);
    else
        eprintf_local_uint_padding_left(nb_s, mod);
}